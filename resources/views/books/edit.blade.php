    ...
    <div class="container">
        ...
        <form action="{{ route('updateBook', ['id' => $book->id]) }} }}" method="POST" enctype="multipart/form-data">
            ...
            <div class="form-group">
                ...
                <input type="text" name="title" value= "{{$book->title}} "     id="title" value="{{ old('title') }}" class="form-control" placeholder=""
            ...
            <div class="form-group">
                ...
                <textarea  type="text"   name="description" value="{{ old('description') }}" id="description" class="form-control" placeholder=""
                    aria-describedby="helpId" rows="3">{{$book->description}}</textarea>
                @error('description')
                    <div class="alert alert-danger">{{ $message }}</div>
                @enderror
            </div>
            {{-- <div class="form-group">
                ...
                <textarea type="text"   name="blurb" value="{{ old('blurb') }}" id="blurb" class="form-control" placeholder="" aria-describedby="helpId" rows="10">{{$book->blurb}} </textarea>
                @error('blurb')
                    <div class="alert alert-danger">{{ $message }}</div>
                @enderror
            </div> --}}
            <div class="form-group">
                ...
                <textarea type="text"   name="passage" value="{{ old('passage') }}" id="passage" class="form-control" placeholder=""
                    aria-describedby="helpId" rows="10">{{$book->passage}}</textarea>
                @error('passage')
                    <div class="alert alert-danger">{{ $message }}</div>
                @enderror
            </div>
            ...
            <div class="form-group">
                ...
                <input type="float" value= {{$book->price}} name="price" id="price" class="form-control" placeholder="" aria-describedby="helpId">
                @error('price')
                    <div class="alert alert-danger">{{ $message }}</div>
                @enderror
            </div>
            ...
            <p class="text-danger">Elke keer als je iets veranderd moet je de afbeelding opnieuw uploaden. (In de volgende versie verander ik dit wel). </p>
