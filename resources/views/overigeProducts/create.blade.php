...
@section('content')
    <div class="container">
        ...
        <form action="{{ route('storeOverigeProduct') }}" method="POST" enctype="multipart/form-data">
            @csrf
            ...
            <div class="form-group">
                ...
                <textarea type="text" name="description" value="{{ old('description') }}" id="description"
                    class="form-control" placeholder="" aria-describedby="helpId" rows="3"></textarea>
                @error('description')
                    <div class="alert alert-danger">{{ $message }}</div>
                @enderror
            </div>
            ...
            <div class="form-group custom-file">
                <input type="file" class="custom-file-input" id="main_image" name="main_image"
                    accept="image/*">
                <label class="custom-file-label" id="main_image_label"
                    for="main_image">main_image</label>
                @error('main_image')
                    <div class="alert alert-danger">{{ $message }}</div>
                @enderror
            </div>
